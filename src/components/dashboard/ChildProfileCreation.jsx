...
const ChildProfileCreation = () => {
  ...
  const handleNextStep = () => {
    setStep(prevStep => {
      ...
      if (prevStep === 1 && !selectedAvatar) return prevStep;
      ...
      if (prevStep === 2 && (!profileData.name && !profileData.initials || !profileData.ageGroup)) return prevStep;
  ...
  const handleInitialsChange = (e) => {
    setProfileData(prev => ({ ...prev, initials: e.target.value }));
  };

  const handleAgeGroupChange = (group) => {
    setProfileData(prev => ({ ...prev, ageGroup: group }));
  };
  ...
  const handleIdentificationTypeChange = (type) => {
    setProfileData(prev => ({ 
      ...
      name: type === 'Nombre' ? prev.name : '',
      initials: type === 'Iniciales' ? prev.initials : ''
    }));
  };

  const handleParentalConsent = () => {
    setProfileData(prev => ({ ...prev, parentalConsent: !prev.parentalConsent }));
  };
  ...
  const handleSubmit = () => {
    ...
    const newChildProfile = {
      ...
      name: profileData.identificationType === 'Nombre' ? profileData.name : profileData.initials,
      // Extraer el primer número del rango de edad como aproximación
      age: parseInt(profileData.ageGroup.split('-')[0]),
      grade: determineGrade(profileData.ageGroup), // Función auxiliar
      avatar: profileData.avatarImage, // O usar la ruta en vez de la importación
      lastActivity: "Nuevo",
      progress: 0,
      skills: {
        attention: 0,
    ...
    // Función para estimar el grado escolar basado en la edad
    function determineGrade(ageGroup) {
      if (ageGroup === '5-7 años') return "1° Primaria";
      if (ageGroup === '8-10 años') return "3° Primaria";
      if (ageGroup === '11-13 años') return "6° Primaria";
      return "Primaria";
    }
  
    // Guardar en localStorage para que Dashboard pueda acceder
    ...
    console.log('Child Profile Data:', newChildProfile);
    navigate('/dashboard');
  };
        ...
        <div className="avatar-carousel">
          ...
          <div className="avatar-grid">
            {pageAvatars.map((avatar) => (
              <div 
                key={avatar.name} 
                className={`avatar-option ${selectedAvatar === avatar.name ? 'selected' : ''}`}
    ...
    return (
      <div className="profile-step">
        ...
        <div className="identification-type-buttons">
          <Button 
            type={profileData.identificationType === 'Nombre' ? 'primary' : 'option'}
            onClick={() => handleIdentificationTypeChange('Nombre')}
          >
            Nombre
          </Button>
          <Button 
            type={profileData.identificationType === 'Iniciales' ? 'primary' : 'option'}
        ...
        <div className="age-group-section">
          ...
          <div className="age-group-buttons">
            {['5-7 años', '8-10 años', '11-13 años'].map((group) => (
              <Button 
                key={group}
          ...
          <div className="creation-navigation">
            ...
            {step === 2 && (
              <Button 
                type="primary" 
                onClick={handleNextStep}
                disabled={(!profileData.name && !profileData.initials) || !profileData.ageGroup}
              >
                Siguiente
              </Button>
            )}
