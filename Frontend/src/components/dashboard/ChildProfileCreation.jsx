...
import "./ChildProfileCreation.css";
...
const ChildProfileCreation = () => {
  ...
  // Lista de avatares disponibles para selección con sus imágenes
  const avatars = [
    { name: "Astronauta", image: avatar1 },
    { name: "Explorador", image: avatar2 },
    { name: "Científico", image: avatar3 },
    { name: "Artista", image: avatar4 },
    { name: "Deportista", image: avatar5 },
    { name: "Músico", image: avatar6 },
    { name: "Aventurero", image: avatar7 },
    { name: "Inventor", image: avatar8 },
    { name: "Detective", image: avatar9 },
    { name: "Superhéroe", image: avatar10 },
    { name: "Piloto", image: avatar11 },
    { name: "Marinero", image: avatar12 },
  ];
  ...
  // Manejo de navegación del carrusel de avatares
  const handleNextPage = () => {
    setAvatarPage((prev) => (prev + 1) % totalPages);
  };

  const handlePrevPage = () => {
    setAvatarPage((prev) => (prev - 1 + totalPages) % totalPages);
  };
  ...
  // Navegación entre pasos del flujo de creación
  const handleNextStep = () => {
    setStep((prevStep) => {
      // Validación antes de avanzar al siguiente paso
      if (prevStep === 1 && !selectedAvatar) {
        console.log("Error: No se ha seleccionado avatar");
        return prevStep;
      }
      ...
      if (prevStep === 1 && selectedAvatar) {
        ...
        setProfileData((prev) => ({
          ...
          avatarImage: selectedAvatarObj.image,
        }));
      }
      
      // Validación corregida para el paso 2
      if (prevStep === 2) {
        const hasValidIdentification = profileData.identificationType === "Nombre" 
          ? profileData.name.trim() !== "" 
          : profileData.initials.trim() !== "";
        
        if (!hasValidIdentification || !profileData.ageGroup) {
          console.log("Error: Faltan datos en paso 2", {
  ...
  // Función corregida para cambio de tipo de identificación
  const handleIdentificationTypeChange = (type) => {
    setProfileData((prev) => ({
      ...prev,
      identificationType: type,
      // No borrar los datos existentes, solo cambiar el tipo
    }));
  };
  ...
  const handleParentalConsent = () => {
    setProfileData((prev) => ({
      ...prev,
      parentalConsent: !prev.parentalConsent,
    }));
  };

  const handleSubmit = () => {
    try {
      // Crear un nuevo perfil en formato compatible con Dashboard
      const newChildProfile = {
        id: Date.now(), // Generar ID único basado en timestamp
        name: profileData.identificationType === "Nombre"
          ? profileData.name.trim()
          : profileData.initials.trim(),
        age: profileData.ageGroup, // Mantener el rango completo
      ...
      console.log("Perfil creado exitosamente:", newChildProfile);
      
      // Navegar al dashboard
      navigate("/dashboard");
    } catch (error) {
      console.error("Error al crear perfil:", error);
      alert("Hubo un error al crear el perfil. Por favor, intenta de nuevo.");
    }
  };

  // Función para validar si se puede avanzar desde el paso 2
  ...
  /**
   * Renderiza el grid de selección de avatares (Paso 1)
   */
  const renderAvatarGrid = () => {
    const startIndex = avatarPage * avatarsPerPage;
    const endIndex = startIndex + avatarsPerPage;
    const pageAvatars = avatars.slice(startIndex, endIndex);
    ...
    return (
      <div className="avatar-selection-container">
        ...
        <div className="avatar-carousel">
          ...
          <button
            className="avatar-nav-btn right"
  ...
  /**
   * Renderiza el formulario de identificación (Paso 2)
   */
    ...
    return (
      <div className="profile-step">
        ...
        <div className="identification-type-buttons">
          <Button
            type={
              profileData.identificationType === "Nombre" ? "primary" : "option"
            }
            onClick={() => handleIdentificationTypeChange("Nombre")}
          >
            Nombre
          </Button>
          <Button
            type={
              profileData.identificationType === "Iniciales"
                ? "primary"
                : "option"
            }
            onClick={() => handleIdentificationTypeChange("Iniciales")}
          >
            Iniciales
          </Button>
        </div>
        ...
        {profileData.identificationType === "Nombre" ? (
          <div className="input-container">
            ...
            />
            <p className="hint">
              Solo utilizaremos el primer nombre para identificar a tu hijo/a en
              la plataforma
            </p>
          </div>
        ) : (
          <div className="input-container">
            ...
            />
            <p className="hint">
              Puedes usar 2-4 caracteres como iniciales para mayor privacidad
            </p>
          </div>
        )}
        ...
        <div className="age-group-section">
          ...
          <div className="age-group-buttons">
            {["6-7 años", "7-8 años", "8-9 años"].map((group) => (
              <Button
                key={group}
                type={profileData.ageGroup === group ? "primary" : "option"}
  ...
  /**
   * Renderiza el paso de consentimiento parental (Paso 3)
   */
      ...
      <div className="consent-step">
          ...
          <div className="consent-main">
            <div className="info-box">
              ...
              <div className="info-content">
                ...
                <p>
                  En cumplimiento con las leyes de protección de datos de
                  menores, necesitamos tu consentimiento explícito como padre,
                  madre o tutor legal para crear un perfil para tu hijo/a en
                  nuestra plataforma.
                </p>
              </div>
            </div>
            ...
            <div className="profile-content">
              ...
              <div className="summary-details">
                <div className="summary-grid">
                  ...
                  <p>
                    <strong>Tipo:</strong> {profileData.identificationType}
                  </p>
                  <p>
                    <strong>Avatar:</strong> {profileData.avatar}
                  </p>
                  <p>
                    <strong>Grupo de edad:</strong> {profileData.ageGroup}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="consent-checkbox-container">
          <label className="consent-checkbox-modern">
            <input
              type="checkbox"
              checked={profileData.parentalConsent}
              onChange={handleParentalConsent}
            />
            ...
            <div className="checkbox-text">
              <span className="checkbox-title">
                Doy mi consentimiento como padre/madre o tutor legal
              </span>
              <span className="checkbox-description">
                Al marcar esta casilla, confirmo que soy el padre/madre o tutor
                legal del menor y autorizo la creación de este perfil.
              </span>
            </div>
          </label>
        </div>
  ...
  /**
   * Selecciona qué paso renderizar según el estado actual
   */
  const renderStep = () => {
    switch (step) {
      case 1:
  ...
  return (
    <div className="child-profile-creation-wrapper">
      <div className="child-profile-creation">
        ...
        {/* Solo mostrar botones de navegación en los pasos 2 y 3 */}
        {step > 1 && (
          <div className="creation-navigation">
            <Button type="tertiary" onClick={handlePreviousStep}>
              Atrás
            </Button>
            ...
            {step === 3 && (
              <Button
                ...
                disabled={!profileData.parentalConsent}
              >
                Continuar
              </Button>
            )}
